      ...
      xhttp.onreadystatechange = function () {
        if (this.readyState == 4 && this.status == 200) {
          this.response.forEach(e => {
            $('<div class="fc-event ui-draggable ui-draggable-handle">' + e.title +' '+e.subcode+' <br>'+e.lectname+'<br>'+e.dn+' '+e.start+'-'+e.end+'<br> '+ e.venue+ '</div>').insertBefore($('#sidebar-wrapper p'))
          })
          sList = this.response;
          i = 0;
          ...
          $('#sidebar-wrapper .fc-event').each(function () {
            ...
            $(this).data('event', {
              title: (/([^S]+\s?[^S\s]+)/g).exec($(this).text())[0], // use the element's text as the event title
              subcode: (/(S\S+)\s/g).exec($(this).text())[1], // use the element's text as the event title
              s:(/.*\s(\d\d:\d\d)-(\d\d:\d\d)/g).exec($(this).text())[1],
              e:(/.*\s(\d\d:\d\d)-(\d\d:\d\d)/g).exec($(this).text())[2],
              color: "purple",
        ...
        eventReceive: function (event, delta, revertFunc) {
          ...
          if (event._id == "sec") {   //update time for sec
            for (let index = 0; index < sList.length; index++) {
              if (event.subcode == sList[index].subcode && event.s == sList[index].start && event.e == sList[index].end ){
                console.log('in',sList[index].start)
                v = sList[index].start
                w = sList[index].end
